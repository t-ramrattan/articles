...
public class ArticlesDAOTest {
    ...
    @Test
    public void should_return_empty_optional_when_article__id_has_no_match() {
        final long id = 123;
        doReturn(null).when(this.operations).findById(eq(id), eq(Article.class));
        assertFalse(this.daoUnderTest.getArticle(id).isPresent());
    }
